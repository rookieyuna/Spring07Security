...
<div class="container">
	...
	<c:choose>
		<c:when test="${not empty user_id }">
			${user_id }님, 반갑습니다
			<!-- 
			시큐리티 설정에 logout-url="/security2/logout" 이와같으므로
		...
		만약 해당 설정값을 추가했다면 컨텍스트 루트 경로를 제외한 
		나머지 경로를 통해 action 속성을 기술해야 한다.
		 -->
		 <!-- 방법1]JSTL의 url태그를 사용한 경우 컨텍스트루트 경로가 자동으로 붙는다. -->
			...
			<form:form name="loginFrm" action="../myLoginAction" method="post">
				...
				password-parameter="pass" 이와 같이 설정되어 있으므로 name속성은
				아래와 같이 기술한다.
				 -->
				<p>
